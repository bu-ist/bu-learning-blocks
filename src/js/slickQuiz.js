...
( function( $ ) {
	$.slickQuiz = function( element, options ) {
		...
		const quizValues = plugin.config.json ?
			plugin.config.json :
		...
		let questions = plugin.config.randomSortQuestions ?
		...
		plugin.method = {
			...
			setupQuiz: function( options ) {
				...
				// Setup questions
				let quiz = $( '<ol class="' + questionGroupClass + '"></ol>' ),
					count = 1;
				...
				for ( i in questions ) {
					if ( questions.hasOwnProperty( i ) ) {
						...
						const answerHTML = $(
							'<ul class="' + answersClass + '"></ul>'
						);

						// Get the answers
						const answers = plugin.config.randomSortAnswers ?
						...
						let selectAny = question.select_any ?
								question.select_any :
								false,
							forceCheckbox = question.force_checkbox ?
								...
								false,
							checkbox =
								( truths > 1 && ! selectAny ) || forceCheckbox,
							inputName =
								$element.attr( 'id' ) + '_question' + ( count - 1 ),
							inputType = checkbox ? 'checkbox' : 'radio';

						if ( count == quizValues.questions.length ) {
							nextQuestionClass =
								nextQuestionClass + ' ' + lastQuestionClass;
						}
						...
						let nextText = plugin.config.nextQuestionText;
						if (
							plugin.config.completeQuizText &&
							count == questionCount
						) {
			...
			checkAnswer: function( checkButton, options ) {
				...
				let questionLI = $( $( checkButton ).parents( _question )[ 0 ] ),
				...
				const correctResponse = plugin.method.compareAnswers(
					trueAnswers,
					selectedAnswers,
					selectAny
				);
				...
				questionLI
					.find(
						correctResponse ? _correctResponse : _incorrectResponse
					)
					.show();
			...
			nextQuestion: function( nextButton, options ) {
				...
				let currentQuestion = $( $( nextButton ).parents( _question )[ 0 ] ),
				...
				if ( nextQuestion.length ) {
					currentQuestion.fadeOut( 300, function() {
						...
						if (
							...
							kN( key, 1 ).apply( null, [] );
						} // 1st notch on key must be passed even if there's no "back" button
					} );
				} else {
					kN( key, 1 ).apply( null, [] ); // 1st notch on key must be on both sides of if/else, otherwise key won't turn
			...
			backToQuestion: function( backButton, options ) {
				...
				let questionLI = $( $( backButton ).parents( _question )[ 0 ] ),
				...
				if ( responses.css( 'display' ) === 'block' ) {
					...
					const prevQuestion = questionLI.prev( _question );
			...
			completeQuiz: function( options ) {
				...
				let score = $( _element + ' ' + _correct ).length,
					displayScore = score;
			...
			calculateLevel: function( correctAnswers ) {
				let percent = ( correctAnswers / questionCount ).toFixed( 2 ),
					level = null;
		...
		plugin.init = function() {
			...
			const _qnid = $element.attr( 'id' ) + '-name';
	...
	$.fn.slickQuiz = function( options ) {
		return this.each( function() {
			if ( undefined === $( this ).data( 'slickQuiz' ) ) {
				const plugin = new $.slickQuiz( this, options );
